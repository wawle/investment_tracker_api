...
export const advancedPriceProvider = async (
  url: string,
  priceCellIndex: number
) => {
      ...
      (allowedCurrencies: string[], priceCellIndex: number) => {
        ...
        rows.forEach((row) => {
          ...
          if (tickerName) {
            ...
            const priceCell: any = row.querySelectorAll("td")[priceCellIndex]; // Access the specified column by index
